...
Subscription import, manual refresh and scheduled refresh use `ConfigurationStore.ImportSubscriptionAsync` and send `User-Agent: clash.meta/v1.19.31`.

This matches the bundled Mihomo release's default `GlobalUA` (`"clash.meta/" + C.Version`):
https://github.com/MetaCubeX/mihomo/blob/v1.19.31/config/config.go#L502
