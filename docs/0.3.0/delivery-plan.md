...
0.3.0 按“先保护本机路径，再增加自动化，最后接入远程”的顺序开发。任何里程碑都必须保持仓库可构建、相关测试可运行、本机日常代理路径可用。
...
| 0.3.0-alpha.2 | SSID 自动切换完整纵向切片 |
| 0.3.0-alpha.3 | 远程 Mihomo 端点完整纵向切片 |
| 0.3.0-beta.1 | 集成、迁移、安全、性能与 UI 硬化 |
...
关键路径是：目标模型 → 配置切换事务 → SSID；目标模型 → 统一传输 → 远程端点。远程功能不得在能力隔离和统一 TLS 完成前抢跑。
...
- INetworkContextSource、WindowsNetworkContextSource、NetworkRuleStore 和 NetworkSwitchPolicyEngine。
- 当前 SSID 权限状态、多 Wi-Fi 接口消歧、睡眠恢复和网络变化监听。
- 规则编辑、默认配置、启用确认、暂停、恢复自动和手动覆盖 UI。
- 5 秒防抖、30 秒冷却、latest-wins 事件管线。
- 受保护 SSID 存储、日志去标识和规则目标删除处理。
- 通过 ConfigurationSwitchCoordinator 执行所有自动切换。

退出标准：

- 纯策略测试覆盖精确匹配、默认值、无匹配、重复规则、目标丢失、手动覆盖、权限拒绝、模糊网络和冷却。
- 快速网络事件只产生一次最终切换。
- 睡眠恢复、Wi-Fi 到 Ethernet、Ethernet 到 Wi-Fi、位置权限允许/拒绝有真实 Windows 证据。
- 核心原本停止时自动切换仍保持停止。
- 自动切换失败不破坏手动配置选择或本机网络。
...
| W-030-008 | 网络上下文与权限适配 | NET-001、NET-002、NET-010 至 NET-012 |
| W-030-009 | 规则存储与纯策略 | NET-003 至 NET-006、NET-013 至 NET-015 |
| W-030-010 | 网络事件管线和手动覆盖 | NET-007 至 NET-009 |
...
每个合并请求或变更批次至少标注一个工作项和对应需求编号，避免“做了功能但无法证明满足哪个验收条件”。
...
- NetworkSwitchPolicyEngine 的所有状态与优先级组合。
...
3. 假 NetworkContextSource：确定性重放 Wi-Fi、Ethernet、权限、多接口、抖动和 Resume 序列。
...
- Windows 位置权限允许、拒绝、稍后开启和策略不可用。
- Wi-Fi 连接切换、多网卡、Ethernet 切换、睡眠恢复。
...
| SSID | 权限、抖动、Resume、多接口、手动覆盖、默认配置实机证据 |
...
| Windows SSID 权限在不同系统策略下表现不一致 | 中 | 高 | 权限作为一等状态；实机矩阵；功能默认关闭 |
| 快速网络事件触发重启风暴 | 中 | 高 | latest-wins、5 秒防抖、30 秒冷却、同目标去重 |
...
1. 保留 M0、M1 和 M2，发布只包含基础收口与 SSID 的 0.3.0。
2. 将 M3 远程端点整体移动到 0.3.1。
...
- 发布说明明确 HTTP 端点风险、SSID 权限要求和相同 SSID 无法区分的限制。

### 12.2 回退

- 0.3.0 新功能数据使用独立文件，0.2.0 不应读取或删除。
- 回退前先在 0.3.0 关闭自动切换并切回本机目标。
...
- 代码实现覆盖正式需求，且没有通过 UI 隐藏替代 Core 安全边界。
