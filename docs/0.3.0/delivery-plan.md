...
> 状态：0.3.0 正式版范围冻结；SSID/Wi-Fi 功能明确顺延
> 对应需求：[产品需求](product-requirements.md)
> 对应设计：[技术设计](technical-design.md)

> **0.3.0 范围声明：** 0.3.0 不包含 Wi-Fi/SSID 配置切换、SSID 读取、网络变化监听、网络规则编辑或位置权限处理。SSID 相关工作项和验收项仅保留为后续版本参考，不是 0.3.0 的发布门禁。
...
0.3.0 的关键路径是：目标模型 → 配置切换事务，以及目标模型 → 统一传输 → 远程端点。SSID 自动切换不是本版本关键路径，相关网络上下文和权限工作整体顺延；远程功能不得在能力隔离和统一 TLS 完成前抢跑。
...
本里程碑从 0.3.0 移出，整体顺延到后续版本。0.3.0 不创建 `WindowsNetworkContextSource`，不读取 SSID，不监听网络事件，不提供网络规则编辑，不申请或引导位置权限；`INetworkContextSource`、`NetworkRuleStore`、策略引擎及其测试仅作为未来版本设计/代码预留，不计入 0.3.0 交付。

后续版本重新立项时，仍需单独验证权限、抖动、恢复、多接口、手动覆盖、规则存储和配置切换回滚；不得把本版本的 UI 隐藏或合成 smoke 结果解释为这些能力已交付。
...
- 0.3.0 发布前连续日常使用期间没有代理残留、重启风暴或跨目标状态污染。
...
每个合并请求或变更批次至少标注一个工作项和对应需求编号，避免“做了功能但无法证明满足哪个验收条件”。W-030-008 至 W-030-010 在 0.3.0 中保持延期状态，不得作为当前版本已交付功能统计。

## 6. 测试计划

### 6.1 单元测试

必须覆盖（SSID 相关项仅为后续版本参考，不是 0.3.0 门禁）：

- 后续版本再覆盖 NetworkSwitchPolicyEngine 的所有状态与优先级组合；0.3.0 不把它作为交付项。
...
3. 后续版本使用假 NetworkContextSource 确定性重放 Wi-Fi、Ethernet、权限、多接口、抖动和 Resume 序列；0.3.0 不创建生产网络上下文源。
...
- Windows 位置权限、Wi-Fi/SSID 连接切换、多网卡和 Ethernet 自动切换属于后续版本，不纳入 0.3.0 实机验收。
...
| SSID / Wi-Fi | 明确排除在 0.3.0；不读取 SSID、不监听网络变化、不提供网络规则或位置权限流程 |
...
- 若后续版本重新启用 SSID，位置权限拒绝不得导致崩溃、循环弹窗或本机功能不可用；该项不是 0.3.0 门禁。
...
| 后续版本重新引入 Windows SSID 权限时表现不一致 | 后续版本 | 高 | 0.3.0 不启用；重新立项时以权限状态和实机矩阵验收 |
| 后续版本快速网络事件触发重启风暴 | 后续版本 | 高 | 0.3.0 不启用；重新立项时使用 latest-wins、防抖、冷却和同目标去重 |
...
- 发布说明明确 HTTP 端点风险，并明确声明 0.3.0 不包含 SSID/Wi-Fi 自动切换；SSID 权限和相同 SSID 无法区分等内容留待后续版本。

### 12.2 回退

- 0.3.0 新功能数据使用独立文件，0.2.0 不应读取或删除。
- 回退前先切回本机目标；SSID 自动切换未在 0.3.0 启用，不需要执行 SSID 自动切换关闭流程。
...
- 代码实现覆盖 0.3.0 的正式需求，且没有通过 UI 隐藏替代 Core 安全边界；延期的 NET 需求不计入本版本完成定义。
