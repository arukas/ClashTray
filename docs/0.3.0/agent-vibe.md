...
6. 如果涉及 Windows UI、Wi-Fi、代理、TUN、Service 或安装器，做真实环境验收并记录未验证项。
...
### 4. 接入 SSID，但让它只是一个“决策来源”

SSID 不拥有重启逻辑。它只输出 ConfigurationSwitchRequest。

拆成：

- WindowsNetworkContextSource：当前连接、权限、多接口、Resume。
- NetworkRuleStore：DPAPI SSID、规则和默认配置。
- NetworkSwitchPolicyEngine：纯函数。
- 事件管线：latest-wins、5 秒防抖、30 秒冷却。
...
- 权限拒绝是正常状态，不是异常风暴。
- 手动选择压过当前网络会话。
- 同 SSID 不重复切；相同 SSID 的不同网络明确告知无法区分。
- 规则坏了，手动路径仍然好用。

完成信号：fake source 可以确定性重放所有事件；真实 Windows 上权限、Wi-Fi/Ethernet、Resume 和抖动通过。
...
- 原代理和设置都在。
- 什么新功能都不会擅自开启。
- 开启 SSID 自动切换后，网络稳定变化只触发一次可解释的事务；失败会回到旧配置。
