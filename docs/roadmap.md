...
- 把发布拆成 Full、NoCET 两种自包含 x64 变体，所有安装器生成 SHA-256 sidecar，并提供 Mihomo GPLv3 来源元数据（0.3.2 起移除试验性的 Mini framework-dependent 变体）。
- 增加 CI 与 tag / 手动发布 workflow，发布前自动运行完整构建和测试。
- 收紧 `.gitignore`，忽略核心二进制、构建输出、订阅/日志、压缩包、签名材料和运行数据。

## 下一轮验证顺序

1. 在干净 Windows 10 22H2 和 Windows 11 x64 上安装 Full、NoCET。
