...
2. 构建四种变体，检查文件存在、SHA-256 sidecar 和体积报告。
...
- `release.yml` 在 `v*.*.*` tag 或手动输入版本时运行 CI 检查，随后构建 Full / NoCET / NoCore / Framework，下载同一 Mihomo tag 的 source archive，将其加入 `SHA256SUMS.txt`，并用仓库的 `GITHUB_TOKEN` 创建 Release。
