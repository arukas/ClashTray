...
The runtime buffers at most 500 UI log entries and folds adjacent identical lines. Configuration and subscription data stay under `%LOCALAPPDATA%\ClashTray`; service-owned runtime data stays under `%PROGRAMDATA%\ClashTray`. Controller secrets are protected with Windows DPAPI.
